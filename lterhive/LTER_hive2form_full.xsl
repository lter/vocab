...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="SKOSConcepts">
    <xsl:element name="strong">Select the keywords you wish to use in your metadata from the suggested list</xsl:element>
    <xsl:element name="br"></xsl:element>
When there are similar terms (e.g., humidity, relative humdity), select only the most specific term that applies (e.g., relative humidity). 
    <xsl:element name="br"></xsl:element>
<xsl:element name="form">
  <xsl:attribute name="action">http://vocab.lternet.edu/lterhive/LTERhive2list.php</xsl:attribute>
  <xsl:attribute name="method">post</xsl:attribute>
  <xsl:element name="input">
    <xsl:attribute name="type">hidden</xsl:attribute>
    <xsl:attribute name="name">thesaurus</xsl:attribute>
    <xsl:attribute name="value">$schema</xsl:attribute>
  </xsl:element>
...
<xsl:element name="tr">
  ...
  <xsl:attribute name="value">Use Selected Terms</xsl:attribute>
  </xsl:element>
</xsl:element> 
  </xsl:template>
